...
class Weka():
    ...
    # TODO: definir e adicionar características
    def create_audio_file(self, file_name):
        self.header = '''
                        @relation caracteristicas\n
                        @attribute apu_body real
                        @attribute apu_pants real
        ...
        for index, audio in enumerate(list(self.list_directory_files())):
            Logger.log(f'Extracting characteristics from {audio}')

            features = ReadImage().read(f'{self.images_directory}/{audio}')
            features[6] = "Cat" if features[6] == 0.0 else "Dog"
